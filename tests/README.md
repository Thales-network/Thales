# Functional testing for Thales

This folder contains a set of functional tests desgined for Thales network.
...
You can also add the node's logs to the output using the `THALES_LOG` env variable. Ex:

```
THALES_LOG="warn,rpc=trace" npm run test
```
...
the left bar of VSCode and make sure **Launch Thales Node (Linux)** is selected in the top
dropdown. **Build & Launch Thales Node (Linux)** will trigger the build before launching the node.
