...
import { BINARY_PATH, DISPLAY_LOG, THALES_LOG, SPAWNING_TIME } from "./constants";
...
// It is used when a test file contains multiple describeDevThales. Those are
// executed within the same PID and so would generate a race condition if started
// at the same time.
let nodeStarted = false;

// This will start a thales dev node, only 1 at a time (check every 100ms).
// This will prevent race condition on the findAvailablePorts which uses the PID of the process
export async function startThalesDevNode(): Promise<{
  p2pPort: number;
  rpcPort: number;
  wsPort: number;
  ...
  runningNode.on("error", (err) => {
    if ((err as any).errno == "ENOENT") {
      console.error(
        `\x1b[31mMissing Thales binary ` +
          `(${BINARY_PATH}).\nPlease compile the Thales project\x1b[0m`
      );
    } else {
      console.error(err);
    }
  ...
  await new Promise<void>((resolve) => {
    const timer = setTimeout(() => {
      console.error(`\x1b[31m Failed to start Thales Test Node.\x1b[0m`);
