...
import { startThalesDevNode } from "./dev-node";
import {
  provideWeb3Api,
  provideEthersApi,
  providePolkadotApi,
...
export function describeDevThales(title: string, cb: (context: DevTestContext) => void) {
  describe(title, function () {
    ...
    let thalesProcess: ChildProcess;

    // Making sure the Thales node has started
    before("Starting Thales Test Node", async function () {
      this.timeout(SPAWNING_TIME);
      const init = !DEBUG_MODE
        ? await startThalesDevNode()
        : {
            runningNode: null,
            p2pPort: 19931,
            wsPort: 19933,
      ...
      thalesProcess = init.runningNode;
    ...
    after(async function () {
      ...
      if (thalesProcess) {
        await new Promise((resolve) => {
          thalesProcess.once("exit", resolve);
          thalesProcess.kill();
          thalesProcess = null;
        });
      }
