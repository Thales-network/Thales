...
use log::info;
use thales_runtime::{AccountId, Block};
	...
	match id {
		...
		"" => Err(
			"You have not specified what chain to sync. In the future, this will default to \
				Thales mainnet. Mainnet is not yet live so you must choose a spec."
				.into(),
		),
...
impl SubstrateCli for Cli {
	fn impl_name() -> String {
		"Thales Parachain Collator".into()
	}
	...
	fn support_url() -> String {
		"https://github.com/Thales-network/thales/issues/new".into()
	}
...
impl SubstrateCli for RelayChainCli {
	...
	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		match id {
			"thalesbase_alpha_relay" => Ok(Box::new(RococoChainSpec::from_json_bytes(
				&include_bytes!("../../specs/alphanet/rococo-embedded-specs-v7.json")[..],
			)?)),
			"thalesbase_stage_relay" => Ok(Box::new(RococoChainSpec::from_json_bytes(
				&include_bytes!("../../specs/stagenet/rococo-embedded-specs-v7.json")[..],
			)?)),
			// If we are not using a thales-centric pre-baked relay spec, then fall back to the
