...
use thales_extensions_evm::runner::stack::TraceRunner as TraceRunnerT;
...
pub const VERSION: RuntimeVersion = RuntimeVersion {
	spec_name: create_runtime_str!("thales"),
	impl_name: create_runtime_str!("thales"),
	authoring_version: 3,
	spec_version: 35,
	impl_version: 1,
...
pub struct ThalesGasWeightMapping;

impl pallet_evm::GasWeightMapping for ThalesGasWeightMapping {
...
impl pallet_evm::Config for Runtime {
	type FeeCalculator = ();
	type GasWeightMapping = ThalesGasWeightMapping;
	...
	type Precompiles = precompiles::ThalesPrecompiles<Self>;
...
impl_runtime_apis! {
	...
	impl thales_rpc_primitives_debug::DebugRuntimeApi<Block> for Runtime {
		fn trace_transaction(
			extrinsics: Vec<<Block as BlockT>::Extrinsic>,
			transaction: &EthereumTransaction,
			trace_type: thales_rpc_primitives_debug::single::TraceType,
		) -> Result<
			thales_rpc_primitives_debug::single::TransactionTrace,
		...
		fn trace_block(
			...
			Vec<
				thales_rpc_primitives_debug::block::TransactionTrace>,
				sp_runtime::DispatchError
			> {
			use thales_rpc_primitives_debug::{single, block, CallResult, CreateResult, CreateType};
	...
	impl thales_rpc_primitives_txpool::TxPoolRuntimeApi<Block> for Runtime {
		fn extrinsic_filter(
