...
# 44 for ws
#
# Ex: USER_PORT=20000 scripts/run-thalesbase-standalone.sh
# will open port 21542, 21543, 21544

# Loading binary/specs variables
source scripts/_init_var.sh

if [ ! -f "$THALES_BINARY" ]; then
  echo "Thales binary $THALES_BINARY is missing"
...
EXECUTABLE=$THALES_BINARY
if [ ! -z "$PERF" ]; then
  EXECUTABLE="$PERF $THALES_BINARY"
fi
